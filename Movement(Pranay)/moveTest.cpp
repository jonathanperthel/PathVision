...
void Custom::RobotControl() 
{
    ...
    if(motiontime > 4000 && motiontime < 5000){
        cmd.eular[2] = 1 * 1.5708; // Yaw in rad
        cmd.yawSpeed = 1; // Rotation speed of the body 
    }
    if(motiontime > 5000 && motiontime < 6000){
        cmd.mode = 3; // velocity walking
    }
    ...
    if(motiontime > 13000 && motiontime < 14000){
        cmd.eular[2] = - 2 * 1.5708; // Yaw in rad
        cmd.yawSpeed = 1; // Rotation speed of the body 
    }
    
    if(motiontime > 14000 && motiontime < 18000){
        cmd.eular[2] = - 3 * 1.5708; // Yaw in rad
        cmd.yawSpeed = 1; // Rotation speed of the body 
    }
    if(motiontime > 18000 && motiontime < 19000){
        cmd.eular[2] = - 4 * 1.5708; // Yaw in rad
        cmd.yawSpeed = 1; // Rotation speed of the body 
    }
    if(motiontime > 19000 && motiontime < 20000){
        cmd.eular[2] = 0; // Yaw in rad
        cmd.yawSpeed = 1; // Rotation speed of the body 
    }

    if(motiontime > 20000 && motiontime < 26000){
        cmd.eular[2] = 1.5708; // Yaw in rad
        cmd.yawSpeed = 1; // Rotation speed of the body 
    }
